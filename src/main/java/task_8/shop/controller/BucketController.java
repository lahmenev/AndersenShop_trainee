package task_8.shop.controller;
...
import task_8.shop.service.BucketService;
...
public class BucketController {
     ...
     */
    @RequestMapping(value = "/insert/{id}&{name}&{currency}&{price}&{amount}&{nameCountry}", method = RequestMethod.GET)
    public ModelAndView showAddToBucketForm(@PathVariable int id, @PathVariable String name, @PathVariable String currency,
                                            @PathVariable int price, @PathVariable int amount, @PathVariable String nameCountry) {
        return new ModelAndView("addToBucketForm", "product",
                new Product(id, name, currency, price, amount, new Country(nameCountry)));
    }
